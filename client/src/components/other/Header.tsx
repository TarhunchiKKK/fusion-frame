    ...
    return(
        <header className="mx-auto bg-gradient-to-r from-indigo-500 to-blue-500">
            <nav className="py-4 flex justify-between items-center">
                { haveIcons &&  <img src="/icons/search_icon.svg" alt="Поиск" onClick={openSearchModal} className="w-12 h-12 ml-4 rounded-full hover:bg-indigo-700"></img> }
                 { !haveIcons && <div>  </div> }
                ...
                { haveIcons && <img src="/icons/plus_bold.svg" alt="Добавить каталог" onClick={openPathsModal} className="w-10 h-10 mr-4 rounded-full hover:bg-blue-700"></img> }
                { !haveIcons && <div>  </div>}
            </nav>
        </header>
    )
